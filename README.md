...
문장을 클릭하거나 드래그해 선택하면 하이라이트/메모 메뉴가 자동으로 뜬다. 하이라이트는 문장에 형광펜 배경을 칠하고, 메모는 문장 아래 메모 상자로 남는다. 모든 주석은 폴더에 `notes.json`(데이터)과 `NOTES.md`(마크다운)로 저장되며, 재추출 버튼 옆의 `하이라이트·메모` 버튼으로 마크다운 형식으로 한 번에 볼 수 있다.
